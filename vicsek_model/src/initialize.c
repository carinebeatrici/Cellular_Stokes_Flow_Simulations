   ...
   if(SORTEIO==QUADRADO)
	...
	for(i=0;i<N0;i++)
	  {   
	     ...
	     boid[i].x  = Lx * drand48();
	     boid[i].y  = Ly * drand48();
	     radius = sqrt((boid[i].x-L_CENTRO_X)*(boid[i].x-L_CENTRO_X) + (boid[i].y-L_CENTRO_Y)*(boid[i].y-L_CENTRO_Y));
	...
	while(fscanf(arquivo_saida,"%li %d",&aux_time_step,&n_read)!= EOF)
	  {
	     ...
	     for(i=0;i<n_read;i++)
	       {
		  contador++;
		  aux_time_step = fscanf(arquivo_saida,"%f %f %f %f %d %*d",&boid[i].x,&boid[i].y,&boid[i].vx,&boid[i].vy,&boid[i].box);
