...
int main (void)
{
   ...
   retorno += posix_memalign((void**)&vizinhos_old, 16, MAX_VIZ*N*sizeof(int));
   retorno += posix_memalign((void**)&vizinhos, 16, MAX_VIZ*N*sizeof(int));
   retorno += posix_memalign((void**)&num_vizinhos, 16, N*sizeof(int));
   retorno += posix_memalign((void**)&num_vizinhos_old, 16, N*sizeof(int));
   retorno += posix_memalign((void**)&num_different, 16, N*sizeof(int));
   ...
   while (time_step < TIME_FINAL)
	...
	clock_init = clock();
	delta_n = update_position(boid, sum_vx, sum_vy, sum_fx, sum_fy, n, last_id);
...
//		  SaveImageDensity(boid, box_for_line, box_for_row,  imagem, vx_mean, vy_mean, phi_local, angle_local, n);
		  ...
		  solid_liquid = SolidLiquid(boid, time_step, n, vizinhos_old, vizinhos,
                                             num_vizinhos, num_vizinhos_old, num_different);
                  printf("Salvando posicoes %li %f %d %f %d \n",time_step,phi,n,solid_liquid, last_id);
		  clock_end = clock();
		  tempo_file += clock_end - clock_init;
		  clock_init = clock();
...
//		  SaveImageDensity(boid, box_for_line, box_for_row,  imagem, vx_mean, vy_mean, phi_local, angle_local, n);
		  ...
		  printf("Salvando posicoes  t: %li phi: %f Delta: %f n: %d front: %f last: %d delta_n: %d last_id: %d\n",time_step,phi,solid_liquid,n, max_x, count_last, delta_n, last_id);
		  clock_end = clock();
		  tempo_file += clock_end - clock_init;
		  clock_init = clock();
	       }
